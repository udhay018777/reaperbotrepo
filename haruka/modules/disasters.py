...
from haruka.modules.helper_funcs.chat_status import whitelist_plus, dev_plus, sudo_plus
...
disasters = """Reaper has bot access levels we call as *"Disaster Levels"*
\n*God* - Only one exists, bot owner. 
Owner has complete bot access, including bot adminship in chats Reaper is at.
\n*Sudo* - Have super user access, can gban, manage disasters lower than them and are admins in Reaper.
\n*Support* - Have access go globally ban users across Reaper.
\n*Whitelist* - Same as wolves but can unban themselves if banned.
\n*Disclaimer*: The disaster levels in Reaper are there for troubleshooting, support, banning potential scammers.
Report abuse..
"""
...
def addsupport(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id in SUDO_USERS:
        rt += "This person is demoted from sudo to support :)"
    ...
    if user_id in WHITELIST_USERS:
        rt += "This user is now a Support User"
...
def addwhitelist(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id in SUDO_USERS:
        rt += "This member is now demoted to Whitelist user."
        data['sudos'].remove(user_id)
        SUDO_USERS.remove(user_id)
    ...
    if user_id in WHITELIST_USERS:
        message.reply_text("This user is already Whitelisted.")
        return ""
...
def removesudo(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id in SUDO_USERS:
        message.reply_text("Promoted this gey to sudo!")
...
def removesudo(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id in SUDO_USERS:
        message.reply_text("Demoted this gey!")
        SUDO_USERS.remove(user_id)
        data['sudos'].remove(user_id)
...
def removewhitelist(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_id in WHITELIST_USERS:
        ...
        message.reply_text("This user is not Whitelisted!")
        return ""
