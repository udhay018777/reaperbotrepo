...
def mute(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not user_id:
        message.reply_text(tld(chat.id, "I am fed up now. You don`t even know how to use commands. Mention a user whom you want to mute."))
        return ""

    if user_id == bot.id:
        message.reply_text(tld(chat.id, "Nice Joke! I will kill you if you try muting me again"))
        return ""
    ...
    if member:
        ...
        if user_id in SUDO_USERS:
            message.reply_text(tld(chat.id, "Oh! that`s one of my sudo users. I won`t do anything against them. FUCK YOU!."))
            ...
            message.reply_text(tld(chat.id, "This user has already got his butthole sealed {}!").format(chatD.title))
    else:
...
def unmute(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    user_id = extract_user(message, args)
    if not user_id:
        message.reply_text(tld(chat.id, "I will ignore you as you are not mentioning a user."))
        return ""
    ...
    if member.status != 'kicked' and member.status != 'left':
        if member.can_send_messages and member.can_send_media_messages \
            ...
            keyboard = []
            reply = tld(chat.id, "Yep, I will allow {} to talk again in {}! but next time no Mercy will be shown!!").format(mention_html(member.user.id, member.user.first_name), chatD.title)
            message.reply_text(reply, reply_markup=keyboard, parse_mode=ParseMode.HTML)
            return "<b>{}:</b>" \
                   "\n#UNMUTE" \
                   "\n<b>• Admin:</b> {}" \
...
def nomedia(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if member:
        if is_user_admin(chatD, user_id, member=member):
            message.reply_text(tld(chat.id, "Afraid I can't restrict admins! If I could restrict admins I will take over this chat XD"))
