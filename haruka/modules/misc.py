...
def info(bot: Bot, update: Update, args: List[str]):
    ...
    if user.id == OWNER_ID:
        text += tld(chat.id, "\n\nOH! This is my owner. Never say me to do anything against him else you will be FUCKED!!")
    else:
        ...
        if user.id in SUDO_USERS:
            ...
            if user.id in SUPPORT_USERS:
                text += tld(chat.id, "\nThis person is one of my support users! " \
                        "No need to get scared from him! He can just GBAN you and nothing else....!")
