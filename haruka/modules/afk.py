...
def afk(bot: Bot, update: Update):
    ...
    update.effective_message.reply_text(tld(chat.id, f"{fname} is now Dead! SeD reacts only"))
...
def no_longer_afk(bot: Bot, update: Update):
    ...
    if res:
        ...
        update.effective_message.reply_text(tld(chat.id, f"Oh f*ck! The ghost of {firstname} just spawned in the chat!"))
...
def check_afk(bot, update, user_id, fst_name):
    ...
    if sql.is_afk(user_id):
        ...
        if not user.reason:
            res = tld(chat.id, f"{fst_name} is Dead! Type RIP in the chat")
        else:
            res = tld(chat.id, f"{fst_name} is DEAD! because of:\n{user.reason}")
