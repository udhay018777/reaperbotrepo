...
def gban(bot: Bot, update: Update, args: List[str]):
    ...
    if int(user_id) in SUDO_USERS:
        message.reply_text("Oh! A sudo users fight. I am not banning anyone")
        return

    if int(user_id) in SUPPORT_USERS:
        message.reply_text("OOOH someone's trying to gban a support user! Noice This Gey is pro as phakk")
        return
    ...
    send_to_list(bot, SUDO_USERS + SUPPORT_USERS,
                   ...
                   html=True)
    text13 = f"Fucked this gey {ok123} globally 😉 'Peace Out'."
...
def ungban(bot: Bot, update: Update, args: List[str]):
    ...
    message.reply_text("Person has been un-gbanned.Don`t be that much kind stay firm on decisions. kThnxBye :).")
...
def gbanlist(bot: Bot, update: Update):
    ...
    if not banned_users:
        update.effective_message.reply_text("There aren't any gbanned users! Go ban some nibbas.")
        return

    banfile = 'The name of globally banned gays.\n'
...
def check_and_ban(update, user_id, should_message=True):
    if sql.is_user_gbanned(user_id):
        ...
        if should_message:
            update.effective_message.reply_text("Oh SeD Nibba you messed with my sudoers. Now they have restricted you globally! BYE GO ON A VACATION!")
