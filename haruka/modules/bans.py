...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not user_id:
        message.reply_text(tld(chat.id, "Atleast refer a person, I am tired of teaching you everything nigga."))
        return ""
    ...
    try:
        ...
        if excp.message == "Person not found":
            message.reply_text(tld(chat.id, "AH! I think this gey is Mr India"))
            return ""
        else:
            raise
    ...
    if is_user_ban_protected(chat, user_id, member):
        message.reply_text(tld(chat.id, "I can`t ban chat admins, ask the creator to do it and let me sleep."))
        return ""
    ...
    try:
        ...
        message.reply_text(tld(chat.id, "Executed!"))
        return log

    except BadRequest as excp:
        if excp.message == "Reply message not found":
            # Do not reply
            message.reply_text(tld(chat.id, "Executed!"), quote=False)
            return log
        else:
