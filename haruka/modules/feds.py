...
def new_fed(bot: Bot, update: Update):
	...
	if not fednam == '':
		...
		if not x:
			update.effective_message.reply_text("Federation creation failed! Keep in the mind that this rarely happened! Ask in @ReaperBot_official for help!")
			return

		update.effective_message.reply_text("*You have successfully created a new federation! Wew! You are pro *"\
											"\nName: `{}`"\
											"\nID: `{}`"
...
def fed_chat(bot: Bot, update: Update, args: List[str]):
	...
	text = "This chat has been joined to the following federation:"
        ...
        for admin in administrators:
            ...
            if status == "creator":
                ...
                if str(admin.user.id) == str(user.id):
                    ...
                    return
    if fed_id:
        message.reply_text("OWO! You are such a stpid. How the fuck will you join two federations at one chat?")
        return
...
def leave_fed(bot: Bot, update: Update, args: List[str]):
	...
	if getuser in 'creator' or user.id in SUDO_USERS:
		if sql.chat_leave_fed(chat.id) == True:
			update.effective_message.reply_text("This chat is now free from Federation: {}!".format(fed_info['fname']))
		else:
