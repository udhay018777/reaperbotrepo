...
def promote(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not chatD.get_member(bot.id).can_promote_members:
        update.effective_message.reply_text("I can't promote/demote people here! "
                                            "Either I am not admin or you nigga didn`t give me sufficient permissions.")
        exit(1)

    user_id = extract_user(message, args)
    if not user_id:
        message.reply_text(tld(chat.id, "You noob atleast refer a user."))
        return ""
    ...
    if user_id == bot.id:
        message.reply_text(tld(chat.id, "How the fuck I will promote myself! Get an admin to do it for me ASAP else you are a DEAD MAN!."))
        return ""
    ...
    message.reply_text(tld(chat.id, f"Successfully Made admin in *{chatD.title}* Now gib party or you gey!"), parse_mode=ParseMode.MARKDOWN)
...
def demote(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if user_member.status == 'creator':
        message.reply_text(tld(chat.id, "This person CREATED the chat, I won`t do anything against him. You GTFO!"))
        return ""
    ...
    if user_id == bot.id:
        message.reply_text(tld(chat.id, "Why would I demote myself. OWO! you are such a stupid!"))
        return ""
    ...
    try:
        ...
        message.reply_text(
            tld(chat.id, "Could not demote. I might not be admin, or the admin status was appointed by another user, so I can't act upon them! This nigga got some time before he is EXECUTED!")
            )
        return ""
